...
With `sysml2kit>=0.3`, `phased-array-systems>=0.13`, and this library
...
with positive margin (the exact numbers track phased-array-systems and are
asserted in this repo's `pas`-marked test, not here):

```text
PASS  REQ-MARGIN  link_margin_db  >= 3.0    [must]
PASS  REQ-EIRP    eirp_dbw        >= 40.0   [must]
PASS  REQ-SLL     sll_db          <= -20.0  [must]
PASS  REQ-POWER   prime_power_w   <= 450.0  [must]
PASS  REQ-COST    total_cost_usd  <= 60000  [must]
```
